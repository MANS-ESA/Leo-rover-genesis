...
def main():
    ...
    wheel_base = 0.359
    left_wheel_vel = (linear_vel - (angular_vel * wheel_base / 2)) / 0.033 
    right_wheel_vel = (linear_vel + (angular_vel * wheel_base / 2)) / 0.033 

    r0.control_dofs_velocity(
                np.array([left_wheel_vel, right_wheel_vel, left_wheel_vel, right_wheel_vel]),
                dofs_idx,
            )
